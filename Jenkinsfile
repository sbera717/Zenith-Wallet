pipeline{
    ...
    stages{
      ...
      environment {
        DOCKER_IMAGE = "sbera717/transaction-service:${BUILD_NUMBER}"
        ...
        steps {
            withCredentials([string(credentialsId: 'github', variable: 'GITHUB_TOKEN')]) {
                sh '''
                    ...
                    sed -i "s/tag: .*/tag: ${BUILD_NUMBER}/" transaction-chart/values.yaml
                    git add transaction-chart/values.yaml
                    git commit -m "Update values in Helm chart to version ${BUILD_NUMBER}"
                    git push https://${GITHUB_TOKEN}@github.com/${GIT_USER_NAME}/${GIT_REPO_NAME} HEAD:Transaction-Service
                '''
            }
        }
    }
